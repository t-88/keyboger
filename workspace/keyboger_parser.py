        ...
        if not typ in macros_ids:
            ...
            if len(args) == 3:
                ...
                if args[0] == "local":
                    ...
                    # TODO: change macro serpator its the same as like for fish sake
                    if args[1] ==  "https" or args[1] ==  "http":
                        self.online_imgs[args[0]] = args[1] +":"+ args[2]
            elif len(args) == 2:
...
class KeybogerParser:
    ...
    def parse(self,tknz = []):
        ...
        while not self.eof_tknz():
            ...
            ast_elem = self.parse_new_line(tkn)
            if ast_elem:
    ...
    def parse_list(self,tkn):
        if tkn.typ == TokenType.unordered_list:
            ...
            while tkn.typ == TokenType.unordered_list or tkn.typ == TokenType.tab:
                ...
                # keep eating until the end of the line
                content = []
                while not self.eof_tknz() and self.peek_tkn().typ != TokenType.new_line:
                    content.append(self.parse_macro(self.peek()[1]))
                    self.inc()
                

                # save list element
                lists.append(
                    AstElement(AstType.unordered_list,content,data={"depth":depth,"inner":[]})
                    )
                ...
                if self.far_peek(TokenType.new_line):
                    ...
                    # get out there is no more lists 
                    break
    ...
    def parse_ordered_list(self,tkn):
        if tkn.typ == TokenType.ordered_list or tkn.typ == TokenType.list_idxer:
            ...
            while tkn.typ == TokenType.ordered_list or tkn.typ == TokenType.list_idxer or  tkn.typ == TokenType.tab:
                ...
                # save list element
                lists.append(
                    AstElement(AstType.ordered_list,content,data={"idxer":idxer,"depth":depth,"inner":[]})
                    )
    ...
    def parse_macro(self,tkn):
        ...
        if tkn.typ == TokenType.macro_start:
            ...
            if self.far_peek(TokenType.text,1) and self.far_peek(TokenType.macro_end,2):
                ...
                #TODO: remove double-colon token make it text and parse it as text
        ...
        if tkn.typ == TokenType.inline_macro_start:
            ...
            if correct:
                ...
                if self.far_peek(TokenType.inline_macro_end):
                    return AstElement(AstType.inline_macro,content,data={"id":macro_id.strip()})
    ...
    def parse_code_blk(self,tkn):
        ...
        if tkn.typ == TokenType.back_tick:
            ...
            if count == 3:
                ...
                if check_count == 3:
                    self.cur -= 1
                    return AstElement(AstType.code,code.replace("<","&lt;").replace(">","&gt;").replace(" ","&nbsp;")) 
    ...
    def parse_text(self,tkn):
        ...
        # replacing with html symbols
        if tkn and tkn.typ != TokenType.new_line:
            return AstElement(AstType.text,tkn.val.replace("<","&lt;").replace(">","&gt;"))
